...
def mcallester_bound(alpha, beta, delta, predictors, data, verbose=False):
    ...
    if verbose:
        print(f"Empirical risk={tr}, KL={kl}, const={const}")
        print(f"Bound={bound}")
...
# alpha_mca = batch_gradient_descent(mcallester_bound, alpha, (beta, delta, predictors, (train_x, train_y)), lr=1, num_iters=1000)
...
alpha_err = batch_gradient_descent(risk, alpha, (predictors, (train_x, train_y)), lr=1, num_iters=100)
